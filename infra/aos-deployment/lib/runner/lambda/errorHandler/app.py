        ...
        if conatiners:
            for container in conatiners:
                ...
                if exit_code and exit != 0:
                    print(
                        f"Container {container.get('ContainerArn')} failed with {exit_code}"
                    )
                    send_wait_condition_response(
                        "FAILURE", str(exit_code), stopped_reason
                    )
                    return
                elif (
                    not exit_code
                ):  # happens when task failed to be created, e.g., image cannot be pull
                    print(
                        f"Container {container.get('ContainerArn')} failed, {stopped_reason}"
                    )
                    send_wait_condition_response(
                        "FAILURE", str(exit_code), stopped_reason
                    )
