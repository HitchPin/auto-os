...
func InitLogging(req handler.Request) {

	l, err := NewCloudWatchLogsProvider(
		cloudwatchlogs.New(req.Session),
		"HitchPin-AutoOs-RootCA",
	)
	if err != nil {
		panic(fmt.Sprintf("Error: %v, Logging to Stdout", err))
	}
	SetProviderLogOutput(l)
}
