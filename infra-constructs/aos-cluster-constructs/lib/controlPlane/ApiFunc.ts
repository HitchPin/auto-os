...
const MAESTRO_LAMBDA_LOC = process.env["MAESTRO_LAMBDA_LOC"]!;

let lambdaLoc: string;
if (!EXEC_ROOT || !MAESTRO_LAMBDA_LOC) {
  lambdaLoc = "/Users/john/project-maestro/dist/bin/maestro/lambda";
} else {
  lambdaLoc = path.join(EXEC_ROOT, MAESTRO_LAMBDA_LOC);
}
  ...
  constructor(parent: Construct, name: string, props: ApiFuncProps) {
        ...
        "conf-read-writer": new iam.PolicyDocument({
          statements: [
            new iam.PolicyStatement({
              actions: ["s3:*"],
              resources: [
                props.confBucket.arnForObjects(props.confPrefix + "*"),
              ],
            }),
          ],
        }),
    ...
    this.func = new lambda.Function(this, "MaestroFunc", {
      ...
      environment: {
        ...
        CONF_BUCKET: props.confBucket.bucketName,
