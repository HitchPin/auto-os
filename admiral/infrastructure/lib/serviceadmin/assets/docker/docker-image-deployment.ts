...
import { aws_lambda as lambda, CustomResource, Duration, CfnOutput, Token } from 'aws-cdk-lib';
...
const IMAGE_DEPLOYER_LOC = process.env["IMAGE_DEPLOYER_LOC"]!;

let deployerLoc: string;
if (!EXEC_ROOT || !IMAGE_DEPLOYER_LOC) {
  deployerLoc = "/Users/john/project-maestro/dist/bin/handlers/image-deployer/package.zip";
} else {
  deployerLoc = path.join(EXEC_ROOT, IMAGE_DEPLOYER_LOC);
}
...
export class DockerImageDeployment extends Construct {
  ...
  constructor(scope: Construct, id: string, props: DockerImageDeploymentProps) {
    ...
    const onEventHandler = new lambda.Function(this, 'onEventHandler', {
      code: lambda.Code.fromAsset(deployerLoc),
